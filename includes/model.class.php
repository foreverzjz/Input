<?php
	...
	class Model{
		...
		private function getColumn(){
			//判断缓存文件是否存在，如果缓存文件存在，直接获取缓存文件的内容
				...
				$data = $this->query("desc ".$this->tabName);
				foreach($data as $val){
					if($val['Key']=='PRI'){
		...
		public function insert($data=null){
			...
			$val = "'".join("','",$data)."'";
			//组装sql语句
			$this->sql = "insert into ".$this->tabName."($key) values($val)";
			//执行sql语句，返回受影响行数
	...
	//1.0--2.0
		...
		public function select($field='',$where='',$order='',$limit=''){
			...
			$this->sql = "select ".$this->dbPrefix.$this->fields." from ".$this->tabName.$this->where.$this->order.$this->limit;
